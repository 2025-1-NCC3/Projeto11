...
async function createUsuarioMotorista(usuario) {
  try {
    const newMotorista = await MotoristaModel.create({
      cnh: usuario.cnh,
...
exports.deleteUser = async (req, res) => {
  ...
  try {
    console.log("Iniciando exclusão do usuário com ID:", id); // Log para depuração

    // Tenta encontrar o usuário pelo ID
    const usuario = await UsuarioModel.findOne({
      where: { id },
      transaction: t, // Usa a transação aqui
    });

    if (!usuario) {
      await t.rollback(); // Caso o usuário não seja encontrado, faz rollback
      return res.status(404).json({ message: "Usuário não encontrado" });
    }

    // Exclui o motorista se o tipo de usuário for 'motorista'
    if (usuario.tipo_usuario === "motorista") {
      await MotoristaModel.destroy({
        where: { id_usuario: id },
        transaction: t, // Transação aplicada
      });
    }

    // Exclui o passageiro se o tipo de usuário for 'passageiro'
    ...
    // Comita a transação, se tudo ocorrer corretamente
    await t.commit();
